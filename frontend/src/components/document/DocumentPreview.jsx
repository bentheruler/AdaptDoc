...
const DocumentPreview = forwardRef(({ children, editMode, onToggleEditMode, paperSize = 'A4', showWatermark }, ref) => {
  const getDims = () => {
    switch (paperSize) {
    ...
    `}</style>
      ...
      <div
        ...
        style={{
          ...
          width: dims.width, minHeight: dims.height,
      ...
      >
        {showWatermark && (
          <div style={{
            ...
            pointerEvents: 'none', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center',
            overflow: 'hidden'
          }}>
            <div style={{
              transform: 'rotate(-45deg)', fontSize: '140px', color: 'rgba(0,0,0,0.03)', fontWeight: 'bold', whiteSpace: 'nowrap'
            }}>
              DRAFT
            </div>
          </div>
        ...
        <div style={{ outline: 'none', marginTop: editMode ? 30 : 0, paddingBottom: '20px', cursor: editMode ? 'text' : 'default', transition: 'margin-top 0.2s', width: '100%', flex: 1, display: 'flex', flexDirection: 'column' }}>
          {children}
        </div>
      </div>
    </div>
